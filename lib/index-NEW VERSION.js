...
class KafkaProxy {
    ...
    _connectToKafka(consumerGroup, topic) {
        ...
        console.log("DEBUG Consumer Group: "+consumerGroup);
        ...
        console.log("DEBUG Consumer information: "+this.consumers[consumerGroup][topic].groupId);
        return this.consumers[consumerGroup][topic].init();
    }

    _registerTopic(ws) {
        console.log(`got new incoming web socket connection: ${ws.upgradeReq.url}`);
        ...
        if (ws.upgradeReq.url) {
            ...
            if (this.clients[consumerGroup][topic]) {
                console.log(`client already connected to ${consumerGroup} / ${topic}. disconnecting`);
    ...
    _startListening(ws, consumerGroup, topic, partition, requestOffset) {
        ...
        this.clients[consumerGroup][topic].ws.on('close', () => {
            console.log(`close socket event raised for ${consumerGroup} / ${topic}`);
            this._disconnectClient(consumerGroup, topic, partition, () => {                 
                console.log(`socket is closed and cleanup completed for ${consumerGroup} / ${topic}`);
            });
        });
        ...
        console.log(`[${consumerGroup} / ${topic}] storing incoming connection, querying offset`);
        ...
        if (!requestOffset) {
            ...
            return this._getOffset(consumerGroup, topic, partition).then((result) =>{ 
                if (result[0].error) {
                    ...
                    console.log(`Error fetching offset for existing topic ${result[0].error}`);
                }

                console.log(`[${consumerGroup + '/' + topic}] Got offset data: ${result[0].offset}`);
                ...
                return this.consumers[consumerGroup][topic].subscribe(topic, partition, { offset: (result[0].offset)-10}, (messageSet, topicCombo, partition) => {
                //console.log("DEBUG:  TIME LATEST-OFFSET:"+ {time: kafka.LATEST_OFFSET} );
                //return this.consumers[consumerGroup][topic].subscribe(topic, partition, { time: kafka.LATEST_OFFSET}, (messageSet, topicCombo, partition) => {
                    //console.log("DEBUG:  topic: "+topic+"  LATEST-OFFSET:"+kafka.LATEST_OFFSET);
                    //console.log("DEBUG !!!!!! ");
                    this.clients[consumerGroup][topic].messageHandler(messageSet, consumerGroup + '/' + topic, partition)
                    console.log("DEBUG:  topic: "+topic+" TIME LATEST-OFFSET-1:"+ this.consumers[consumerGroup][topic].offset-1);
                });

            }).catch((err) => {
                console.log(`.catch(error) fetching offset for existent topic: ${err}`);
                ...
                console.log(`.error(error) fetching offset for existent topic: ${err}`);
                return this._disconnectClient(consumerGroup, topic, partition, () => {
                    // delete the consumer so we can try again next time
                    delete this.consumers[consumerGroup][topic];
                    return;
    ...
    _getOffset(consumerGroup, topic, partition) {
        ...
        /*
        var offset = new kafka.Offset(consumerGroup);
        ...
        //return this.consumers[consumerGroup][topic].fetchLatestOffsets
    }
    ...
    _batchMessageHandler(messageSet, topicCombo, partition) {
        ...
        //console.log(`[${consumerGroup} / ${topic}] received offset: ${messageSet[batchSize].offset}`);
    ...
    _outOfRangeHandler(err) {
        console.log(`Got offset error on kafka: ${JSON.stringify(err) }`);
    }

    _errorHandler(err) {
        console.log(`got error on kafka: ${err}`);
    }
    ...
    _handleSendResponse(err, client, consumerGroup, topic, partition, offset, batchSize) {
        if (err) {
            ...
            console.log(`could not send to: ${consumerGroup} / ${topic} as web socket has disconnected`);
        }
        else {
            if (offset % 10000 == 0) {                
                //console.log(`[${consumerGroup} / ${topic}] sent offset: ${offset}`);
            }
            ...
            if (this.clients[consumerGroup][topic]) {
                ...
                .catch((ex) => {
                    console.log(`error setting offset for consumer group [${consumerGroup}], topic [${topic}]: ${ex}`);                   
                });
    ...
    _disconnectClient(consumerGroup, topic, partition, callback) {
        ...
        console.log('removed client from active connection pool');        
        ...
        return this.consumers[consumerGroup][topic].unsubscribe([topic], partition).then(() => {
            ...
            if (this.offsets[consumerGroup][topic]) {
                ...
                console.log(`Latest offset is ${this.offsets[consumerGroup][topic]}, writing to kafka`)
            ...
            if (client.ws.readyState != WebSocket.CLOSING && client.ws.readyState != WebSocket.CLOSED) {
                console.log(`force closing websocket ${consumerGroup + '/' + topic}`);
                ...
                client.ws.on('close', () => {
                    ...
                    console.log(`force close of websocket ${consumerGroup + '/' + topic} completed`);
                    return callback();
                });
            }
            else {
    ...
    _verifyClient(info, callback) {
        ...
        (('basic ' + this.auth) != info.req.headers.authorization && ('Basic ' + this.auth) != info.req.headers.authorization)) {
            console.log(`Unauthorized client connection for ${info.req.url}. Rejecting 401`)
    ...
    _recordMetrics() {
        ...
        Object.keys(this.clients).forEach((consumerGroup) => {
            ...
            Object.keys(this.clients[consumerGroup]).forEach((topicName) => {
                console.log(`******* Topic / Partition: ${topicName} / ${this.clients[consumerGroup][topicName].partition} *********`);
                console.log(`[${topicName}] latest offset: ${this.offsets[consumerGroup][topicName]}`);
                console.log(`[${topicName}] backlog: ${this.clients[consumerGroup][topicName].backlog}`);
                console.log(`[${topicName}] received: ${this.clients[consumerGroup][topicName].received}`);
                console.log(`[${topicName}] sent: ${this.clients[consumerGroup][topicName].sent}`);
                let duration = (new Date().getTime() - this.clients[consumerGroup][topicName].createdAt.getTime()) / 1000;
                console.log(`[${topicName}] duration: ${duration}`);
                console.log(`[${topicName}] throughput: ${this.clients[consumerGroup][topicName].sent / duration}`);
            });
        });
        console.log(`************************************`);
    }
    ...
    /*
    //Metodo bloccante
    syncGetTopic(upperLeft_x, upperLeft_y, lowerRight_x, lowerRight_y){
